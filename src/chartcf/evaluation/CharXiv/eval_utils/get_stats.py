...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir',
                        ...
                        help='Directory containing CharXiv metadata JSON files')
    args = parser.parse_args()

    descriptive_score_path = f"{args.output_name}/{args.score_prefix}{args.model_name}-descriptive_{args.split}.json"
    reasoning_score_path = f"{args.output_name}/{args.score_prefix}{args.model_name}-reasoning_{args.split}.json"

    image_meta = json.load(
        open(f"{args.data_dir}/image_metadata_{args.split}.json"))
    descriptive_meta = json.load(
        open(f"{args.data_dir}/descriptive_{args.split}.json"))
    reasoning_meta = json.load(
        open(f"{args.data_dir}/reasoning_{args.split}.json"))
    ...
    if os.path.exists(reasoning_score_path):
        ...
        json.dump(
            reasoning_stats,
            open(
                f"{args.output_name}/{args.stats_prefix}{args.model_name}-reasoning_{args.split}.json",
                "w"),
            indent=4)
    ...
    if os.path.exists(descriptive_score_path):
        ...
        json.dump(
            descriptive_stats,
            open(
                f"{args.output_name}/{args.stats_prefix}{args.model_name}-descriptive_{args.split}.json",
                "w"),
            indent=4)
