...
def get_reasoning_result_gpt(client, prompt, model="gpt-4o", max_retries=5):
    curr_retries = 0
    max_tokens = 256
...
async def async_get_reasoning_result_gpt(client, prompt, model="gpt-4o", max_retries=5):
    """Async version of get_reasoning_result_gpt for concurrent API calls."""
    curr_retries = 0
    max_tokens = 256
    while curr_retries < max_retries:
        try:
