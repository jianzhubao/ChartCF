...
def get_descriptive_result_gpt(client, prompt, length, model="gpt-4o", max_retries=10):
    curr_retries = 0
    max_tokens = 256
...
# Async version for async evaluation
async def async_get_descriptive_result_gpt(client, prompt, length, model="gpt-4o", max_retries=10):
    """Async version of get_descriptive_result_gpt for concurrent API calls."""
    curr_retries = 0
    max_tokens = 256
    while curr_retries < max_retries:
        try:
            ...
            # Try to parse JSON, with fallback for invalid escapes
            try:
                content = json.loads(response_content)
            except json.JSONDecodeError as json_error:
                # If we encounter invalid escape sequences, try to fix them
                if 'Invalid \\u' in str(json_error) or 'Invalid control character' in str(json_error) or 'escape' in str(json_error).lower():
                    print(f"Attempting to fix invalid escape sequences in JSON response")
                    # Fix common invalid escape sequences by escaping single backslashes
                    # that are not part of valid JSON escape sequences
                    # Valid JSON escapes: \", \\, \/, \b, \f, \n, \r, \t, \uXXXX
                    fixed_response = re.sub(r'\\(?!["\\/bfnrtu])', r'\\\\', response_content)
