"""
Convert SWIFT inference results back to ChartQA evaluation format
...
ChartQA expected format (json list):
[
    {
        "question": "...",
        "gt_answer": "...",
...
def convert_swift_to_chartqa(swift_output_file, dataset_file):
    """
    Convert SWIFT inference results to ChartQA format
    ...
    Returns:
        List of dicts in ChartQA evaluation format
    """
    ...
    # Convert to ChartQA evaluation format
    chartqa_results = []
    ...
    for swift_result, data_item in zip(swift_results, dataset):
        ...
        chartqa_results.append(result)

    return chartqa_results


def main():
    parser = argparse.ArgumentParser(description='Convert SWIFT results to ChartQA format')
    ...
    # Convert results
    chartqa_results = convert_swift_to_chartqa(
        args.swift_output,
        args.dataset
    )

    print(f"Total results: {len(chartqa_results)}")

    # Save results
    print(f"Saving results to: {args.output}")
    with open(args.output, 'w', encoding='utf-8') as f:
        json.dump(chartqa_results, f, indent=4, ensure_ascii=False)
