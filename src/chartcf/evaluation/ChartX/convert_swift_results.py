"""
Convert SWIFT inference results back to ChartX evaluation format
...
ChartX expected format (json list):
[
    {
        "question": "...",
        "gt_answer": "...",
...
def convert_swift_to_chartx(swift_output_file, dataset_file):
    """
    Convert SWIFT inference results to ChartX format
    ...
    Returns:
        List of dicts in ChartX evaluation format
    """
    ...
    # Convert to ChartX evaluation format
    chartx_results = []
    ...
    for swift_result, data_item in zip(swift_results, dataset):
        ...
        result = {
            ...
            'image': data_item['_img'],
        }

        chartx_results.append(result)

    return chartx_results


def main():
    parser = argparse.ArgumentParser(description='Convert SWIFT results to ChartX format')
    ...
    # Convert results
    chartx_results = convert_swift_to_chartx(
        args.swift_output,
        args.dataset
    )

    print(f"Total results: {len(chartx_results)}")

    # Save results
    print(f"Saving results to: {args.output}")
    with open(args.output, 'w', encoding='utf-8') as f:
        json.dump(chartx_results, f, indent=4, ensure_ascii=False)
