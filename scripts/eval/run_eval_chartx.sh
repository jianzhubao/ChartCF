...
CHARTX_DATA_PATH="data/public_benchmarks/ChartX/ChartX_annotation_test.json"
CHARTX_IMAGE_DIR="data/public_benchmarks/ChartX/ChartX_png"
SWIFT_DATA_DIR="data/public_benchmarks/ChartX/swift_data"
SWIFT_DATASET="${SWIFT_DATA_DIR}/test.json"
SAVE_NAME="${CHECKPOINT_DIR}/ChartX"
...
python src/chartcf/evaluation/ChartX/prepare_swift_dataset.py \
    --input_file "$CHARTX_DATA_PATH" \
    --image_dir "$CHARTX_IMAGE_DIR" \
...
python src/chartcf/evaluation/ChartX/convert_swift_results.py \
    --swift_output "$SWIFT_OUTPUT" \
    --dataset "$SWIFT_DATASET" \
    --output "$FINAL_OUTPUT"

python src/chartcf/evaluation/ChartX/evaluate.py \
